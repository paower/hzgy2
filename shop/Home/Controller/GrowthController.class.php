...
public function ok()
	...
	public function upgrade()
	{
		...
		if($b == 0){
			...
			$order1['uid']=$id;
			$order1['up'] = $uplevel;
			$order1['shouid'] =  getoktop($id);
			$order1['money'] = 100;
			$order1['id']=upgrade_id();
			$order1['type'] = 1;
			$res1 = M('upgrade')->add($order1);
			if(gettop($id)!=getoktop($id)){
				$lou[] = gettop($id);
				$jid = getoktop($id);
				M('user')->where("userid = $jid")->setInc('jianlou',1);
			}
			//升级奖订单
			if($uplevel == 1){
				$order2['shouid'] = 778904;
			}else{
				$order2['shouid'] = getoktop($id,$uplevel);	
			}
			$order2['uid']=$id;
			$order2['up'] = $uplevel;
			$order2['money'] = 99;
			...
			$order2['type'] = 2;
			$res2 = M('upgrade')->add($order2);
			if($uplevel!=1 && gettop($id,$uplevel)!=getoktop($id,$uplevel)){
				$lou[] = gettop($id);
				$jid = getoktop($id,$uplevel);
				M('user')->where("userid = $jid")->setInc('jianlou',1);
			}
			//见点奖订单
			$order3['uid']=$id;
			$order3['up'] = $uplevel;
			$order3['shouid'] =  getoktop($id,10,true);
			$order3['money'] = 100;
			$order3['id']=upgrade_id();
			$order3['type'] = 3;
			$res3 = M('upgrade')->add($order3);
			if(gettop($id,10,true)!=getoktop($id,10,true)){
				$lou[] = gettop($id);
				$jid = getoktop($id,10,true);
				M('user')->where("userid = $jid")->setInc('jianlou',1);
			}
		...
		$where3['type'] = 3;
		$where3['state'] = 0;
		$indent_info3 = M('upgrade')->where($where3)->find();
		$this->assign('indent_info3',$indent_info3);
		$this->display();
		die;
	}
